...
public class Table implements java.io.Serializable,Cloneable {
	...
	List<List<Column>> uniqueIndexList = new ArrayList<List<Column>>();
   ...
   public Column getRequiredColumnBySqlName(String sqlName) {
	...
	public int getNotPkColumnSize() {
		return getNotPkColumns().size();
	}
	
	
	
	public List<List<Column>> getUniqueIndexList() {
		return uniqueIndexList;
	}

	public void setUniqueIndexList(List<List<Column>> uniqueIndexList) {
	...
	public String getActionUrlName() {
	    if(StringHelper.isBlank(className)) {
	        String removedPrefixSqlName = removeTableSqlNamePrefix(sqlName);
	        return removedPrefixSqlName.replaceAll("_", "-");
	    }else {
	    	return className;
	    }
	}
