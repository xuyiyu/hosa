$(function() {
	...
	var queryUrl = ctx + "/webadmin/marketing/coupon/coupon!list.htm";
	var getUrl = ctx + "/webadmin/marketing/coupon/coupon!input.htm";
	var saveUrl = ctx + "/webadmin/marketing/coupon/coupon!save.htm";
	var deleteUrl = ctx + "/webadmin/marketing/coupon/coupon!delete.htm";
	
	var serviceQueryUrl = ctx + "/webadmin/marketing/coupon/coupon!loadCategory.htm";
	
	var	departmentsUrl = ctx +  "/webadmin/marketing/coupon/coupon!loadDepartments.htm";
	
	
	var serviceMap = null;
	var departmentsMap = new HashMap();
	...
	var columns = [  "id" ,  "type" ,"createdDepartmentId","settleAccountRules",  "departmentId" ,  "name" ,  "value" ,  "valid","validDays" ,  "amount" ,  "allowDonation" ,  "style" ,  "code" ,  "status" ,  "scope" ,  "conditions" ,  "collectNumber" ,  "usedNumber" ,  "modified"];	
	var searchColumns = [  "type" ,  "departmentId" ,  "name" ,  "value" ,  "valid" ,  "amount" ,  "allowDonation" ,  "style" ,  "code" ,  "status" ,  "scope" ,  "conditions" ,  "collectNumber" ,  "usedNumber" ,  "modified","submitTimeBegin","submitTimeEnd","nameLike"];
			...
			if(responseData.r_code == 0) {
				...
				if(data) {
					...
					if(data.type==1){
						...
						editForm.find("[name=couponValue]").show();
						editForm.find("[name=couponPrize]").hide();
						//editForm.find("[name=amount]").parent(".input").show();
					}else if(data.type==2){
						editForm.find("[name=value]").hide();
						editForm.find("[name=couponPrize]").show();
						editForm.find("[name=couponValue]").hide();
	...
	var loadDepartments = function(callback){
		$.webAjax(departmentsUrl, {}, function(responseData) {
	...
	var bindingEditItem = function(data) {
		...
		editForm.find("[name=createdDepartmentId]").empty();
		$.each(departmentsMap.values(), function(i, o) {
			...
			var selected = "";
			editForm.find("[name=createdDepartmentId]").append("<option value='"+o.id+"' "+selected+" class='item'>"+text+"</option>");
		});
		editForm.find("[name=createdDepartmentId]").select2({
			placeholder: "创建部门",
			allowClear: true
		});
		editForm.find("[name=createdDepartmentId]").select2("val",data.createdDepartmentId);
		
		
		var dataArray = new Array();
		if(departmentsMap){
			$.each(departmentsMap.values(),function(i,o){
				dataArray.push({id:o.id,text:o.name});
			});
			
		}
		editForm.find('[name=grantDepartmentNames]')
		.select2({
			placeholder: "发放部门",
			allowClear: true,
			tags:dataArray});
		...
		if(data.grantDepartmentNames){
			var ids = data.grantDepartmentNames.split(",");
			$.each(ids,function(i,o){
				dbData.push({id:o,text:departmentsMap.get(o).name});
			});
			
		}
		
		
		editForm.find('[name=grantDepartmentNames]').select2("data",dbData);
	...
	var bindingSaveParam = function() {
		...
		var services = editForm.find("[name=selected-service]").find("option");

		$.each(services,function(i,o){
			param["coupon.categories["+i+"].id"] = $(o).val();
		});
		
		param["coupon.allowDonation"] = editForm.find("[name=allowDonationRadio]:checked").val();
		param["coupon.grantDepartmentNames"] = editForm.find("[name=grantDepartmentNames]").val()
	...
	var bindingButtonClick = function() {
		...
		$("[item-id=button-save-submit]").click(function() {
			saveItem(1);
		});
		...
		editForm.find("[name=name]").blur(function(){
			setTimeout(function(){
				var text = editForm.find("[name=name]").val();
				if(!text){
					text = "";
				}
				editForm.find("[name=nameView]").text(text);
			},500);
		});
		
		editForm.find("[name=value]").blur(function(){
			setTimeout(function(){
				var text = editForm.find("[name=value]").val();
				if(!text){
					text = "";
				}
				text = "￥" + text;
				editForm.find("[name=valueView]").text(text);
			},500);
		});
