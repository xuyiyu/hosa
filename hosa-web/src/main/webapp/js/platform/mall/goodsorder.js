$(function() {
	...
	var queryUrl = ctx + "/webadmin/mall/goodsorder/goods-order!list.htm";
	var getUrl = ctx + "/webadmin/mall/goodsorder/goods-order!input.htm";
	var saveUrl = ctx + "/webadmin/mall/goodsorder/goods-order!save.htm";
	var deleteUrl = ctx + "/webadmin/mall/goodsorder/goods-order!delete.htm";
	
	var goodsorderMap = new HashMap();
	...
	var columns = [  "id" ,  "code" ,  "goodsId" ,  "goodsSnapshotId" ,  "price" ,  "transactionPrice" ,  "userId" ,  "status" ,  "description" ,  "modified"];	
	var searchColumns = [  "codeLike" ,  "goodsId" ,  "goodsSnapshotId" ,  "price" ,  "transactionPrice" ,  "userId" ,  "status" ,  "description" ,  "modified"];
	var filterColumns = [  "filter_code" ,  "filter_goodsId" ,  "filter_goodsSnapshotId" ,  "filter_price" ,  "filter_transactionPrice" ,  "filter_userId" ,  "filter_status" ,  "filter_description" ,  "filter_modified"];
	...
	var loadList = function(p) {	
		...
		$.webAjax(queryUrl, param, function(responseData) {		
			...
			if(responseData.r_code == 0) {
				...
				if(data) {
					goodsorderMap = new HashMap();
					$.each(data.result, function(i, o){
						goodsorderMap.put(o.id, o);
	...
	var viewItem = function(tr) {	
		...
		var param = {};
		var dept = goodsorderMap.get(itemId);
	...
	var verifySaveParam = function() {
		editFormValidator = editForm.validate({
				...
				messages : {
					...
					status : {
						required : '请输入状态{1：未付款，2：已付款，3：发货，4：完成，5：取消}'
					},
					modified : {
						required : '请输入最后修改时间'
					},
	...
	var bindingSearchParam = function() {
		...
		$.each(searchColumns, function(i, o) {
			...
			if(val) {
				param["goodsOrderQuery."+o+""] = val;
			} else {
				param["goodsOrderQuery."+o+""] = "";
			}
		});
