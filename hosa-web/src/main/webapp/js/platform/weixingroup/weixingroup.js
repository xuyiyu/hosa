$(function() {
	...
	var queryUrl = ctx + "/webadmin/weixingroup/group/weinxin-groups!list.htm";
	var getUrl = ctx + "/webadmin/weixingroup/group/weinxin-groups!input.htm";
	var saveUrl = ctx + "/webadmin/weixingroup/group/weinxin-groups!save.htm";
	var deleteUrl = ctx + "/webadmin/weixingroup/group/weinxin-groups!delete.htm";
	...
	var loadList = function(p) {	
		...
		$.webAjax(queryUrl, param, function(responseData) {
			...
			if(responseData.r_code == 0) {
				...
				if(data) {
					weixinGroupsMap = new HashMap();
					$.each(data.result, function(i, o){
						weixinGroupsMap.put(o.id, o);
	...
	var viewItem = function(tr) {	
		...
		var param = {};
		var item = weixinGroupsMap.get(itemId);
	...
	var bindingSaveParam = function() {
		...
		$.each(columns, function(i, o) {
			...
			if(val) {
				param["weixinGroups."+o+""] = val;
			} else {
				param["weixinGroups."+o+""] = "";
			}
		});
		return param;
	}
