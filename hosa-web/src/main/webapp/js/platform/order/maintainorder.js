$(function() {
	...
	var queryUrl = ctx + "/webadmin/order/maintain/maintain-order!list.htm";
	var getUrl = ctx + "/webadmin/order/maintain/maintain-order!input.htm";
	var saveUrl = ctx + "/webadmin/order/maintain/maintain-order!save.htm";
	var deleteUrl = ctx + "/webadmin/order/maintain/maintain-order!delete.htm";
	
	var maintainorderMap = new HashMap();
	...
	var loadList = function(p) {	
		...
		$.webAjax(queryUrl, param, function(responseData) {		
			...
			if(responseData.r_code == 0) {
				...
				if(data) {
					maintainorderMap = new HashMap();
					$.each(data.result, function(i, o){
						maintainorderMap.put(o.id, o);
	...
	var viewItem = function(tr) {	
		...
		var param = {};
		var dept = maintainorderMap.get(itemId);
	...
	var deleteItem = function(itemId) {
		...
		if(!$.isEmptyObject(param)) {
			$.confirm("确认删除MaintainOrder？", function() {
	...
	var bindingSaveParam = function() {
		...
		$.each(columns, function(i, o) {
			...
			if(val) {
				param["maintainOrder."+o+""] = val;
			} else {
				param["maintainOrder."+o+""] = "";
			}
		});
	...
	var bindingSearchParam = function() {
		...
		$.each(searchColumns, function(i, o) {
			...
			if(val) {
				param["maintainOrderQuery."+o+""] = val;
			} else {
				param["maintainOrderQuery."+o+""] = "";
			}
		});
	...
	var bindingButtonClick = function() {
		...
		$("[item-id=button-toolbar-modify]").click(function() {
			...
			if(checkedObj.length == 0) {
				$.alert("请选择要修改的MaintainOrder。");
				return;
			} else if(checkedObj.length > 1) {
				$.alert("只能修改一个MaintainOrder。");
				return;
			}
